...
$id = $_REQUEST["id"] ?? '';
$kat = db_fetch($bp, "SELECT * FROM kat_bmn WHERE id_kat = :id", ['id' => $id]);

if (!$kat) {
    echo "Kategori BMN tidak ditemukan.";
    exit;
}
?>
<div id="custom-content" class="modal-block modal-block-md">
    <section class="card">
        <header class="card-header">
            ...
            <h2 class="card-title">Edit Kategori BMN</h2>
        </header>
        <div class="card-body">
            <form id='bmn_kat_edit_form' method='post' action='?p=update&tab=bmn_kat_add' enctype='multipart/form-data' class='form-horizontal'>	
